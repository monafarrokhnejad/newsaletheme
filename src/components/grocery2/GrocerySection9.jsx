...
const GrocerySection9 = ({
    ...
    return <Box className={"section9"}>
        <Carousel totalSlides={3} visibleSlides={1} showDots={true} spacing="0px"
                  showArrowOnHover={true}>
            {testimonials && testimonials.map((data, ind) => <StyledBazarCard key={ind}>
                <StyledFlexBox position="relative" flexWrap="wrap">
                    <StyledQuote sx={{
                        right: 0,
                        top: 0
                    }}/>
                    <StyledGridContainer container spacing={1} sx={{direction: "rtl"}}>
                        <Grid item lg={2} sm={3}>
