...
import { StyledArrowBackButton, StyledArrowNextButton, StyledCarouselProvider, StyledDot, StyledDotGroup, StyledSlider } from "./CarouselStyled"; // component props interface
...
const Carousel = ({
  ...
  return <StyledCarouselProvider style={{direction: "ltr"}} sx={sx} step={step} spacing={spacing} interval={interval} infinite={infinite} isPlaying={autoPlay} totalSlides={totalSlides} currentSlide={currentSlide} visibleSlides={visibleSlides} hasMasterSpinner={hasMasterSpinner} isIntrinsicHeight={isIntrinsicHeight} naturalSlideWidth={naturalSlideWidth || 100} naturalSlideHeight={naturalSlideHeight || 125}>
      ...
      {showDots && <StyledDotGroup className={clsx(dotClass)} dot_margin_top={dotGroupMarginTop} renderDots={props => renderDots({ ...props,
      step,
      dotColor
    })} />}
...
const renderDots = ({
  ...
  for (let i = 0; i < total; i += step) {
    dots.push(<StyledDot dot_color={dotColor} dot_active={currentSlide === i ? i + 1 : 0} onClick={() => handleClick(i, false)} key={(Math.random() * i + Date.now()).toString()} />);
