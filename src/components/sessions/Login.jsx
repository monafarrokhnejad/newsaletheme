...
const StyledCard = styled(({children, passwordVisibility, ...rest}) => <div {...rest}>
    {children}
</div>)(({theme, passwordVisibility}) => ({
    width: 500,
...
const Login = () => {
    ...
    return <StyledCard elevation={3} passwordVisibility={passwordVisibility}>
        <form className="content" onSubmit={handleSubmit}>
            ...
            <Small fontWeight="600" fontSize="12px" color="grey.800" textAlign="center" mb={4.5} display="block">با
                ایمیل و رمز عبور خود وارد شوید</Small>

            <BazarTextField mb={1.5} name="username" label="نام کاربری " placeholder="نام کاربری خودرا وارد کنید"
                            variant="outlined"
                            size="small" type="text" fullWidth onBlur={handleBlur} onChange={handleChange}
                            value={values.username || ""} error={!!touched.username && !!errors.username}
                            helperText={touched.username && errors.username}/>

            <BazarTextField mb={2} name="password" label="رمزعبور" placeholder=" ********* خودرا وارد کنید"
                            autoComplete="on" type={passwordVisibility ? "text" : "password"}
                ...
                endAdornment: <IconButton size="small" type="button" onClick={togglePasswordVisibility}>
                    {passwordVisibility ? <Visibility className="passwordEye" fontSize="small"/> :
            ...
            <BazarButton variant="contained" color="primary" type="submit" fullWidth
                         sx={{mb: "1.65rem", height: 44,color:"white"}}>ورود</BazarButton>
