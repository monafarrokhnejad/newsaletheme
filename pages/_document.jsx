...
import createEmotionCache from "../../theme89$/bazar-react-js/src/createEmotionCache";
...
Bazar.getInitialProps = async ctx => {
  ...
  ctx.renderPage = () => originalRenderPage({
    ...
    enhanceComponent: Component => props => <CacheProvider value={cache}>
            <Component {...props} />
          </CacheProvider>
  });
  ...
  const emotionStyleTags = emotionStyles.styles.map(style => <style data-emotion={`${style.key} ${style.ids.join(" ")}`} key={style.key} // eslint-disable-next-line react/no-danger
